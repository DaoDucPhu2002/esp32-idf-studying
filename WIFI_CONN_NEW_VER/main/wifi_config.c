...
bool is_provisioned(void)
{
    ...
    // Kiem tra ten wifi co trong eeprom hay khong?
    for (int i = 0; i < ap_count; i++)
    {
        char *buff_password = NULL;
        int len = 64;
        app_flash_get_str((char *)wifi_info[i].ssid, buff_password, &len);
        if (buff_password != NULL)
        {
            ESP_LOGW("WIFI", "Read flash");
            bzero(&wifi_config, sizeof(wifi_config));
            memcpy(wifi_config.sta.ssid, wifi_info[i].ssid, strlen((char *)wifi_info[i].ssid));
            memcpy(wifi_config.sta.password, buff_password, strlen((char *)buff_password));
            wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
            ESP_ERROR_CHECK(esp_wifi_init(&cfg));
            ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
            ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
...
uint8_t wifi_manager(void)
{
    ...
    /*Luu  ssid va password khi wifi ket noi dung*/
    app_flash_set_str((char *)wifi_config.sta.ssid, (char *)wifi_config.sta.password);
